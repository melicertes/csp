...
# openssl genrsa -des3 -out common-external-ca.key 4096
# openssl req -new -x509 -days 365 -key common-external-ca.key -out common-external-ca.crt
# csp.intrasoft-intl.private
## Create the Server Key, CSR, and Certificate
#
# openssl genrsa -out csp-external.key 1024 -- No passphrase
# openssl req -new -key csp-external.key -out csp-external.csr
#*.central-csp.athens.intrasoft-intl.private
...
## ON LINUX
# curl -v -s -k --key ./csp-external.key --cert ./csp-external.crt https://es.central-csp.intrasoft-intl.com https://localhost
...
#### Internal
## Create the CA Key and Certificate for signing Client Certs
# openssl genrsa -des3 -out common-internal-ca.key 4096
# openssl req -new -x509 -days 365 -key common-internal-ca.key -out common-internal-ca.crt
# local.csp.athens.intrasoft-intl.private
## Create the Server Key, CSR, and Certificate
#
# openssl genrsa -out csp-internal.key 1024 -- No passphrase
# openssl req -new -key csp-internal.key -out csp-internal.csr
#*.local.central-csp.athens.intrasoft-intl.private
## We're self signing our own server cert here.  This is a no-no in production.
...
## ON LINUX
# curl -v -s -k --key ./csp-internal.key --cert ./csp-internal.crt https://es.local.central-csp.intrasoft-intl.com https://localhost

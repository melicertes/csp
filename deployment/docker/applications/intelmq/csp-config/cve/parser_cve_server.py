...
class CveserverParserBot(Bot):
    ...
    def process(self):
        ...
        self.logger.info("CveserverParserBot 2.0 starts....")
        raw_report = utils.base64_decode(report.get('raw'))
        report_json = json.loads(raw_report)
        self.logger.info("CveserverParserBot report_json %s", report_json)
        for item in report_json:
            self.logger.info("CveserverParserBot item %s", item)
            ...
            if 'id' in item:
                self.logger.info("CveserverParserBot item id %s", item['id'])               
                additional['id'] = item['id']   
                misp_event["Object"][0]["Attribute"][0]["value"] = item['id']
                event.add('event_description.target', item['id'])          
            if 'cvss' in item:
                self.logger.info("CveserverParserBot item cvss %s", item['cvss'])
            ...
            if 'vulnerable_configuration' in item:
                if len(item['vulnerable_configuration']) != 0:
                    self.logger.info("CveserverParserBot item vulnerable_configuration %s", item['vulnerable_configuration'][0])                    
                    misp_event["Object"][0]["Attribute"][3]["value"] = item['vulnerable_configuration'][0]

            #references 
            if 'references' in item:
                if len(item['references']) != 0:
                    self.logger.info("CveserverParserBot item reference %s", item['references'][0])

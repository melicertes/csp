...
public class Cert2 extends AMLoginModule {
	...
	// configurations
	private Map options;
	private Cert2AuthPrincipal userPrincipal;
	private CallbackHandler callbackHandler;
	static final int ldap_version = 3;

	private static final String amAuthCert = "amAuthCert2";
	...
	// attribute and flag to check whether CRLs should be updated from CRL
	// distribution point
	private String amAuthCert_updateCRL;
	private boolean doCRLUpdate = true;

	static {
		String handler = SystemProperties.get(Constants.PROTOCOL_HANDLER, Constants.JSSE_HANDLER);
	...
	private void initAuthConfig() throws AuthLoginException {
		if (options != null) {
			debug.message("Certificate2: getting attributes.");
			// init auth level
			String authLevel = CollectionHelper.getMapAttr(options, "iplanet-am-auth-cert2-auth-level");
			if (authLevel != null) {
				try {
			...
			amAuthCert_securityType = CollectionHelper.getMapAttr(options, "iplanet-am-auth-cert2-security-type");
			amAuthCert_principleUser = CollectionHelper.getMapAttr(options, "iplanet-am-auth-cert-principal-user");
			amAuthCert_principlePasswd = CollectionHelper.getMapAttr(options, "iplanet-am-auth-cert2-principal-passwd");
			amAuthCert_useSSL = CollectionHelper.getMapAttr(options, "iplanet-am-auth-cert2-use-ssl");
			amAuthCert_userProfileMapper = CollectionHelper.getMapAttr(options,
					"iplanet-am-auth-cert2-user-profile-mapper");
			amAuthCert_altUserProfileMapper = CollectionHelper.getMapAttr(options,
					"iplanet-am-auth-cert2-user-profile-mapper-other");
			amAuthCert_subjectAltExtMapper = CollectionHelper.getMapAttr(options,
					"iplanet-am-auth-cert2-user-profile-mapper-ext");
			amAuthCert_chkCRL = CollectionHelper.getMapAttr(options, "iplanet-am-auth-cert2-check-crl");
			if (amAuthCert_chkCRL.equalsIgnoreCase("true")) {
				amAuthCert_chkAttrCRL = CollectionHelper.getMapAttr(options, "iplanet-am-auth-cert2-attr-check-crl");
				...
				amAuthCert_cacheCRL = CollectionHelper.getMapAttr(options, "openam-am-auth-cert2-attr-cache-crl",
						"true");
				if (amAuthCert_cacheCRL.equalsIgnoreCase("false")) {
					doCRLCaching = false;
				}
				amAuthCert_updateCRL = CollectionHelper.getMapAttr(options, "openam-am-auth-cert2-update-crl", "true");
			...
			if (amAuthCert_chkCertInLDAP.equalsIgnoreCase("true")) {
				amAuthCert_chkAttrCertInLDAP = CollectionHelper.getMapAttr(options,
						"iplanet-am-auth-cert2-attr-check-ldap");
			...
			String ocspChk = CollectionHelper.getMapAttr(options, "iplanet-am-auth-cert2-check-ocsp");
			...
			//
			String gwCertAuth = CollectionHelper.getMapAttr(options, "iplanet-am-auth-cert2-gw-cert-auth-enabled");
			...
			if (gwCertAuth == null || gwCertAuth.equals("") || gwCertAuth.equalsIgnoreCase("none")) {
				if (debug.messageEnabled()) {
					debug.message("iplanet-am-auth-cert-gw-cert2-auth-enabled = " + gwCertAuth);
				}
			} else if (gwCertAuth.equalsIgnoreCase("any")) {
				portal_gw_cert_auth_enabled = true;
			} else {
				portalGateways = (Set) options.get("iplanet-am-auth-cert-gw-cert2-auth-enabled");
			...
			amAuthCert_startSearchLoc = CollectionHelper.getServerMapAttr(options,
					"iplanet-am-auth-cert2-start-search-loc");
	...
	/**
	 * Process Certificate based auth request plus oragnisation checking
	 * 
	 * @param callbacks
	 *            for auth
	 * @param state
	...
	public int process(Callback[] callbacks, int state) throws AuthLoginException {
		...
		/*
		 * Based on the certificates presented, find the registered (representation) of
		 * the certificate. If no certificates match in the LDAP certificate directory
		...
		if (amAuthCert_chkCertInLDAP.equalsIgnoreCase("true")) {
			X509Certificate ldapcert = AMCertStore.getRegisteredCertificate(ldapParam, thecert,
					amAuthCert_chkAttrCertInLDAP);
			if (ldapcert == null) {
	...
	private int doJSSRevocationValidation(X509Certificate cert) {
		...
		/**
		 * OCSP validation, this will use the CryptoManager.isCertvalid() method to
		 * validate certificate, OCSP is one of the steps in this process. Here is the
		 * algorith to find OCSP responder: 1. use global OCSP responder if set 2. use
		 * the OCSP responder in user's certificate if presents 3. no OCSP responder The
		 * isCertValid() WON'T perform OCSP validation if no OCSP responder found in
		 * above process.
		 */
		if (ocspEnabled) {
			try {
		...
		if ((ret == ISAuthConstants.LOGIN_SUCCEED) && (crlEnabled || ocspEnabled) && validateCA
				&& !AMCertStore.isRootCA(cert)) {
			/*
			 * The trust anchor is not necessarily a certificate, but a public key (trusted)
			 * entry in the trust-store. Don't march up the chain unless the AMCertStore can
			 * actually return a non-null issuer certificate. If the issuer certificate is
			 * null, then the result of the previous doRevocationValidation invocation is
			 * the final answer.
			 */
			X509Certificate issuerCertificate = AMCertStore.getIssuerCertificate(ldapParam, cert,
					amAuthCert_chkAttrCertInLDAP);
	...
	private void setLdapStoreParam() throws AuthLoginException {
		/*
		 * Setup the LDAP certificate directory service context for use in verification
		 * of the users certificates.
		 */
		try {
			ldapParam = AMCertStore.setLdapStoreParam(amAuthCert_serverHost, amAuthCert_serverPort,
					amAuthCert_principleUser, amAuthCert_principlePasswd, amAuthCert_startSearchLoc,
					amAuthCert_uriParamsCRL, amAuthCert_useSSL.equalsIgnoreCase("true"));
	...
	private void getTokenFromCert(X509Certificate cert) throws AuthLoginException {
		...
		if (!amAuthCert_userProfileMapper.equalsIgnoreCase("none") && (userTokenId == null)) {
	...
	private void getTokenFromSubjectAltExt(X509Certificate cert) throws AuthLoginException {
		try {
			...
			SubjectAlternativeNameExtension altNameExt = (SubjectAlternativeNameExtension) exts
	...
	private void getTokenFromSubjectDN(X509Certificate cert) throws AuthLoginException {
		/*
		 * The certificate has passed the authentication steps so return the part of the
		 * certificate as specified in the profile server.
		 */
		try {
			/*
	...
	public java.security.Principal getPrincipal() {
		if (userPrincipal != null) {
			...
			userPrincipal = new Cert2AuthPrincipal(userTokenId);
			return userPrincipal;
		} else {
			return null;
		}
	...
	private X509Certificate sendCallback() throws AuthLoginException {
		...
		try {
			...
			/*
			 * Allow Cert auth module accepts personal certificate only for following 3
			 * cases : 1. portal_gw_cert_auth_enabled == true : Case of getting cert from
			 * trusted host like sra, distAuth, trusted LB 2. xcb.getReqSignature() == false
			 * : Case of getting cert through ssl client auth enabled port 3.
			 * (xcb.getReqSignature() == true) && (signature != null) : Case of getting cert
			 * together with signature from sdk client
			 */
	...
	private X509Certificate getPortalStyleCert(HttpServletRequest request) throws AuthLoginException {
		...
		if (certParam == null || certParam.equals("")) {
			debug.message("Certificate2: no cert from HttpServletRequest");
			throw new AuthLoginException(amAuthCert, "noCert", null);
		}
		...
		debug.message("Certificate2: CertificateFactory.getInstance.");

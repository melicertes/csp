...
public class EmailService {
	static class MessagePreparatorImpl implements MimeMessagePreparator {
		...
		public void prepare(MimeMessage mimeMessage) throws Exception {
			...
			if (et.getType().equals(EmailTemplateType.INVITATION)) {
				...
				messageHelper.addAttachment("meeting.ics", new ByteArrayDataSource(ics, "ext/calendar"));
			}
		}
	...
	public void prepareAndSend(EmailTemplate et, Meeting meeting) throws IOException {
		...
		if (et.getType().equals(EmailTemplateType.INVITATION)) {
			...
			ics = mailContentBuilder.build(meetingICSBuilder.toString(), m);
		...
		for (Participant p : meeting.getParticipants()) {
			...
			MimeMessagePreparator messagePreparator = new MessagePreparatorImpl(meeting, et, mailContentBuilder, p,
					ics);
			try {
